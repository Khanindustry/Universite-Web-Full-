...
namespace Universite_Web.Areas.admin.Controllers
{
    ...
    public class BlogsController : Controller
    {
        ...
        // GET: admin/Blogs
        public async Task<IActionResult> Index(int page=1,double itemCount=2)
        {
            VmHome model = new VmHome();
            List<Blog> blogs = _context.Blogs.ToList();

            model.PageCount = (int)Math.Ceiling(blogs.Count / itemCount);
            model.Blogs = blogs.Skip((page - 1) * (int)itemCount).Take((int)itemCount).ToList();
            model.Page = page;
        ...
        {
            var blog = await _context.Blogs.FindAsync(id);
            string filePath = Path.Combine(_webHostEnvironment.WebRootPath, "UploadsBlog", blog.Image);
